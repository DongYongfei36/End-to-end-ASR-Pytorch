...
class DlhlpDataset(Dataset):   # Libri-->Dlhlp
    def __init__(self, path, split, tokenizer, bucket_size, ascending=False):
        ...
        # Read text
        text = Parallel(n_jobs=READ_FILE_THREADS)(delayed(read_text)(str(f))
                                                  for f in file_list)
    ...
    def __getitem__(self, index):
        if self.bucket_size > 1:
            ...
            return [(f_path, txt) for f_path, txt in zip(
                self.file_list[index:index +
                               self.bucket_size], self.text[index:index +
                                                            self.bucket_size])]
        else:
